...
## 2026-09-09 補齊骨架章節的草稿、和合本長度線索

- 辨識創世記 34–50 所在的 23 頁；`rom.txt` 骨架裡剩下的空節（創世記 16–20、34–50、傳道書 3、
  哥林多前書 1、3、帖撒羅尼迦後書、提多書、彼得後書、馬可 16、路加 3）全部由 OCR 填入草稿，
  草稿共 4,281 節，只剩路加 3:14 一節空白。`fill_rom` 會替人工錄到一半的章補上其餘節號行。
- 修正 `page-ocr-results.json` 七處打錯（0053、0214、0440、0770、1056–1057、1403–1405、1457），
  都以辨識結果核對過；刪掉不存在的節號行（創世記 20:19–20、約翰 5:48–54）。
- kraken 去彎曲在少數行圖上失敗，原本整行略過（0066 一行、1412 三行）；改成補白邊重試。
- 沒有漢字版的章節加入和合本字數當軟性長度線索（`cuv_lengths.py`、`data/cuv-verse-lengths.json`）：
  藏起所有線索的保留實驗逐節 CER 創世記 2.1% → 1.1%、馬太 2.7% → 1.8%、歷代志上 9.2% → 2.4%，
  併入鄰節的空白節 10 → 2。所有草稿用新切節重寫。
- 使用者的第三批校對決定（`proofread.html` B 類、`syllable_check.html`）尚未收到，兩份報告未動。

## 2026-09-09 節號數字的受限解碼

- 上標節號小又磨損，音節字首樹管不到數字，模型會自信地把 8 讀成 3、20 讀成 0。
  `decode.rescore_numbers` 在 beam 解碼後對每段數字重評分：在該段佔的時間軸區間內，
  對該頁可能出現的每個節號（`common.page_numbers`：頁面對應表的範圍前後各放寬三節，加章號）
  用 CTC 前向演算法算機率，集合外的讀法扣 3 nats，頁眉頁碼不做；再加頁內順序偏置
  （`SEQ_BONUS` 4，接在上一個節號後 1–3 的候選加分），大部分的改善來自這一層。
- 測試集 169 行含數字、整段全對：貪婪與 beam 85.2% → 重評分 87.0% → 加順序偏置 92.3%。
  端到端「節號讀作 …」旗標：創世記 33 → 14、馬太 13 → 4、約翰 4 → 2、歷代志上 5 → 2；
  草稿中有旗標的節 1,419 → 1,115。歷代志上整節正確 75.9% → 77.8%。
- 這些後驗也試過餵給 `assemble.sub` 取代編輯距離，六卷評估都變差（模型讀錯時很有把握，
  正確的節號被判成不可能，反而多出雜訊數字與漏節），已退回並在程式裡註明。
- 241 頁全部重新辨識、草稿全部重新切節；`proofread.html` 與 `syllable_check.html` 依新結果重產
  （完全一致 1,074／1,512，A 27、B 208、C 41、D 95）。
