...
## 2026-09-02 行尾短詞與裁圖幾何

- 使用者指出 OCR 常漏掉行尾的 ē 與 Î-。查出兩個原因：裁圖的欄線偵測與塗白把貼著欄線的字切掉或塗掉；
  自動標籤跟著 OCR 漏字，模型學到「行尾短詞不存在」。修正 `column_rule`（二維長垂直段＋一維尖峰）、
  `erase_rule`（只塗與欄線相連的直欄）、行框邊界，`make_labels` 補回相鄰行標籤之間漏掉的短詞，
  大寫加調符的行加重三倍續訓成 `hinghua_pages3_best`。
- 修正後的測試集（516 行）：`pages2_best` 貪婪 1.26%／beam 1.04% → `pages3_best` 貪婪 1.05%／beam 0.83%，
  整行全對 82%。端到端創世記 1–20 逐節 CER 1.1%、整節 74%、兩字元內 92%；馬太 1.8%／65%／91%。
- 所有草稿（3,466 節）重寫；`proofread.html` 完全一致 1,070／1,512 節，A 類 28。
