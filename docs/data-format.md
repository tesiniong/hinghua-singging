...
原書印錯而正本照規範寫的地方：英文書名（`*` 全部）、章:節（`*` 全部）、原書印法、規範寫法、說明，
tab 分隔。OCR 的訓練標籤與校對比對照原書印法，`assemble.py` 填入時改回規範寫法（見 [ocr.md](ocr.md)「印刷勘誤」）。

## `cuv-verse-lengths.json`

`scripts/tools/ocr/cuv_lengths.py` 從新標點和合本（eBible.org `cmn-cu89t`，公有領域）算出的每節漢字數，
`{英文書名: {"章": [第 1 節字數, 第 2 節字數, …]}}`，標點不算。只給 OCR 切節當長度線索（見
[ocr.md](ocr.md)「切節」），網站不用。
