...
（0017–0022，標籤抄自 rom.txt 的 505 行）行級 CER 1.5%、整行全對 73%；
...
| `recognize.py` | 逐頁辨識：偵測欄溝並塗掉欄線、把前次切割被欄線切碎的行框合併、補回切割漏掉的行、從整頁圖裁行，輸出每行文字與信心，並標記頁眉、頁碼、放大字母 |
...
1. **重新對齊**：如上，一個指令，再跑第三階段訓練即可。做過一次（4,792 行、從 `hinghua_pages_best` 續訓，
   最佳在第 1 輪）：測試 505 行 CER 1.61% → 1.51%、整行全對 72.1% → 72.9%，只是小幅改善，
