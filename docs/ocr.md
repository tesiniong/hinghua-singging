...
修掉的多是調符與行尾少一個字母；剩下的錯多在人名、l／i 這類 LM 幫不上的地方。

**節號數字**另外處理（`decode.rescore_numbers`）。上標節號是小字又常磨損，模型會自信地把 8 讀成 3、
20 讀成 0，音節的字首樹管不到數字。改成解碼後重評分：一行裡每段數字，取它在時間軸上佔的區間
（前後鄰字的發射格之間），對該頁**可能出現的每個節號**用 CTC 前向演算法算機率，集合外的讀法扣
3 nats 的先驗（影像證據夠強時仍可保留，頁眉頁碼行不做）。候選集合是頁面對應表給的節號範圍前後各放寬
三節，加上章號。再加一層頁內順序偏置：行框已依閱讀順序排好，接在上一個節號後面 1–3 的候選加 `SEQ_BONUS`。

| 測試集 169 行含數字，整段數字全對 | |
|---|---|
| 貪婪／beam（原本） | 85.2% |
| ＋預期節號重評分 | 87.0% |
| ＋頁內順序偏置（`SEQ_BONUS` 4） | 92.3% |

端到端「節號讀作 …」的旗標：創世記 33 → 14、馬太 13 → 4、約翰 4 → 2、歷代志上 5 → 2；
逐節 CER 與整節正確率同步小幅改善（歷代志上整節 75.9% → 77.8%）。
每段數字的候選與後驗寫進 `recognized/<page>.json` 的 `nums`，校對時可看模型在猶豫什麼。
註：這些後驗也試過直接餵給切節的對齊成本（取代編輯距離），六卷評估都變差——模型讀錯時往往很有把握，
只信後驗會把正確的節號判成不可能，反而多出雜訊數字與漏節，所以 `assemble.sub` 仍用編輯距離。
...
已有人工正本的節，正本的音節數也當長度線索用（同一個機制）：馬太福音對 517 節正本的逐節 CER
由 11.5% 降到 2.5%。但這個數字對「已知的節」偏樂觀，所以另做保留實驗（藏起長度線索再評估）。

沒有漢字版的章節改用**和合本的字數**當軟性線索：`cuv_lengths.py` 從 eBible.org 的新標點和合本
（公有領域）算出每節漢字數，存成 `data/cuv-verse-lengths.json`；`assemble.py` 換算成音節數
（0.849 × 字數 + 1.7，1,512 節正本擬合，平均差 3.3 音節、九成在 7 音節內）。因為不精確，
對齊時容差放寬（4 + 20%）、單節成本上限 1.0——和合本與本書偶有節的劃分不同（約翰 5:3–4、
古卷缺的節），推估離譜時不能壓過節號本身的證據；和合本字數不到 4 字的節與節數不同的章不用。
節號沒讀出來時，併在一起的經文照推估位置切開，切點吸附到最近的句末標點。
保留實驗（人工正本的章節，把漢字版與正本的長度全部藏起來，模擬沒有任何線索的書卷）：

| 逐節 CER，無任何長度線索 | 沒有和合本線索 | 有 |
|---|---|---|
| 創世記 1–20（440 節） | 2.1% | 1.1% |
| 馬太（517 節） | 2.7% | 1.8% |
| 馬可（49 節） | 3.0% | 1.6% |
| 約翰（149 節） | 2.1% | 1.1% |
| 哥林多前書（62 節） | 4.8% | 2.0% |
...
有精確線索（漢字版或正本）的節結果不變；併入鄰節而空白的節由 10 個減到 2 個。

| 腳本 | 做什麼 |
|---|---|
| `common.py` | 路徑、NFD 叢集切分、去符號比對、向量化的半全域對齊、頁面↔經節對應、行的閱讀順序 |
| `make_labels.py` | 只處理 `rom.txt` 已有經文的頁面；對齊後吸附到詞界，行尾斷詞保留連字號；同欄相鄰兩行標籤之間漏掉的短詞（OCR 常漏讀行尾的 ē、Î-）補回前一行行尾，否則模型會學到「行尾短詞不存在」；貼著缺文邊界或長度差太多的行捨棄。也把前次的人工標註分類（`ok`／`offbyone`／`bad`） |
| `build_dataset.py` | 測試集固定用人工標註過的 0017–0022 頁；其餘自動標籤隨機切 8% 當驗證集；另產生數字加重清單 |
| `recognize.py` | 逐頁辨識：偵測欄溝並塗掉欄線、把前次切割被欄線切碎的行框合併、補回切割漏掉的行、從整頁圖裁行，交給 `decode.py` 解碼，輸出每行文字與信心，並標記頁眉、頁碼、放大字母；kraken 去彎曲失敗的行補白邊重試（否則整行無聲消失） |
| `decode.py` | 受限 CTC beam search：只走合法音節的字首樹，加正本音節 bigram；解碼後把節號數字放回該頁預期的節號重評分（`--digits` 量測、`--seq-bonus`／`--digit-prior` 調參） |
| `evaluate.py` | 含符號／去符號 CER、整行正確率、最常見混淆 |
| `assemble.py` | 丟掉頁眉頁碼、接回行尾連字號、依內文節號切節（用頁面對應表的起始節號校正、容忍上標數字誤讀、漢字版／正本／和合本的長度線索）、章標題轉 `## N`，填入既有的空節號行（人工錄到一半的章會補上其餘節號行）並記進 `ocr-draft.json`；`--evaluate` 與已錄入的人工正本比對；`--no-prior` 不用和合本線索 |
| `cuv_lengths.py` | 從和合本 USFM 算每節字數 → `data/cuv-verse-lengths.json`（已提交，來源檔不在 repo；只在換版本時重跑） |
...
對齊與組裝都已把前一節納入。這張表也決定解碼時的節號候選集合（`common.page_numbers`），
所以表錯了會連辨識一起錯——新書卷開跑前先確認該卷各頁的起始節號遞增。
...
已有該卷的書名行（章節骨架缺的會自動補；66 卷的書名行都在）。頁面對應表偶有打錯（章數少一位、
書名寫成前一卷），先用辨識結果核對該卷各頁的節號序列是否遞增。填入後在網站上對照 📖 圖示校對，改完再 commit。
...
沒有漢字版的書卷（四福音書幾乎都沒有）切節只靠節號序列與和合本推估的長度，所以「節號未辨識出」與
...
| 節號未辨識出 | 該節經文可能併進前一節；有長度線索時會照線索切開（「依長度線索切分」精確、「依和合本字數推估切分」只是估的） |
| 音節數與漢字版差／與和合本推估差 | 切出來的長度與線索不符（後者只標離譜的），多半是切節位置錯 |
